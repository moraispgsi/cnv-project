/* Code_Attribute.java
 * Part of pt.ulisboa.tecnico.meic.cnv.instrument.BIT -- Bytecode Instrumenting Tool
 *
 ...
 *  
 * By downloading pt.ulisboa.tecnico.meic.cnv.instrument.BIT, the User agrees and acknowledges that in no event
 ...
 * inability to use the pt.ulisboa.tecnico.meic.cnv.instrument.BIT software.
 * 
 * pt.ulisboa.tecnico.meic.cnv.instrument.BIT was invented by Han Bok Lee at the University of Colorado in
 * Boulder, Colorado.
 */

package pt.ulisboa.tecnico.meic.cnv.instrument.BIT.lowBIT;
...
public class Code_Attribute extends Attribute_Info {
  ...
  public Code_Attribute(Cp_Info[] constant_pool, DataInputStream iStream,
    ...
    System.err.println("READ pt.ulisboa.tecnico.meic.cnv.instrument.BIT parsing len: " + attribute_length);
    max_stack = (short) iStream.readUnsignedShort();
    max_locals = (short) iStream.readUnsignedShort();
    code_length = iStream.readInt();
    if (System.getProperty("CJKDEBUG3") != null) 
    System.err.println("READ pt.ulisboa.tecnico.meic.cnv.instrument.BIT parsing mxs,mxl,codelen: " + max_stack + " " + max_locals + " " +code_length);
    code = new byte[code_length];
    iStream.readFully(code);
    exception_table_length = (short) iStream.readUnsignedShort();
    if (System.getProperty("CJKDEBUG3") != null) 
    System.err.println("READ pt.ulisboa.tecnico.meic.cnv.instrument.BIT parsing exlen: " + exception_table_length);
    ...
    if (System.getProperty("CJKDEBUG3") != null) 
    System.err.println("READ pt.ulisboa.tecnico.meic.cnv.instrument.BIT parsing attcount: " + attribute_count);
